...
Turn a document into rows, and refuse the readings you cannot trust. A PDF, a Word or OpenDocument file, a paste, a CSV, an OCR page: one engine reads all of them.
...
- **It will never carry an engine of its own.** PDFs go through `pdfjs-dist`, which stays an **optional** peer dependency; a `.docx` and a `.odt` are ZIPs of XML, so they are read with what the platform already has and cost no dependency at all. No OCR engine, no spreadsheet reader, no legacy `.doc`. Anything else, you hand over as bytes or text - and bytes it cannot read are refused by name rather than decoded into mojibake.
